...
def step_execute(request,act):
    ...
    if act==DEMAND:     #If we just implemented "demand" then we are at the start of a new period.
...
This will improve the comparator functionality; the additional stamp will always show differences with the previous stage whilst its predecessor step will always show differences with the previous step
"""
...
def select_comparator(request, period,stage,step):
    ...
    try:
        ...
        comparator=TimeStamp.objects.get(simulation=simulation, period=period, stage=stage,step=step)
