...
def calculate_demand(simulation):
    ...
    for stock in productive_stocks:
        ...
        Trace.enter(simulation,2,f"{Trace.sim_object(stock.stock_owner_name)}'s stock of {Trace.sim_object(stock.commodity_FK.name)} is {Trace.sim_quantity(stock.size)}; production requirement is {Trace.sim_quantity(stock.production_requirement)} and turnover time is {Trace.sim_quantity(turnover_time)}. Demand is increased by {Trace.sim_quantity(stock.demand)} to {Trace.sim_quantity(commodity.demand)}")
        commodity.save()
    ...
    for stock in social_stocks:
        ...
        stock.save()
        commodity.demand += stock.demand
        Trace.enter(simulation,2,f"{Trace.sim_object(stock.social_class_FK.name)}'s demand for {Trace.sim_object(stock.commodity_FK.name)} is increased by {Trace.sim_quantity(stock.demand)} and is now {Trace.sim_quantity(commodity.demand)}")
        commodity.save()
...
def allocate_supply(simulation):
    ...
    for commodity in commodities:
        ...
        related_stocks = commodity.stock_set.filter(time_stamp_FK=current_time_stamp)
...
def calculate_trade(simulation):
    ...
    for buyer_stock in buyer_stocks:
        ...
        # sellers=potential_sellers.filter(commodity_FK=buyer_commodity) 
...
def sale(seller_stock, buyer_stock, seller, buyer):
    ...
    buyer_money_stock=buyer.money_stock
    Trace.enter(simulation,1,f"Sale: {Trace.sim_object(buyer.name)} is buying {Trace.sim_quantity(transferred_stock)} of {Trace.sim_object(seller_stock.commodity_FK.name)} from {Trace.sim_object(seller.name)} at a cost of {Trace.sim_quantity(cost)}")
...
def set_total_value_and_price(simulation):
    ...
    for stock in stocks:
        ...
        stock.save()
        Trace.enter(simulation,4,f"Size of the stock of {Trace.sim_object(stock.commodity_FK.name)} owned by {Trace.sim_object(owner_name)} is {Trace.sim_quantity(stock.size)}. Its value is {Trace.sim_quantity(stock.value)} and its price is {Trace.sim_quantity(stock.price)}")
    ...
    for commodity in Commodity.objects.filter(time_stamp_FK=simulation.current_time_stamp):
        ...
        commodity.save()
        Trace.enter(simulation,3,f"Total size of commodity {Trace.sim_object(stock.commodity_FK.name)} is {Trace.sim_quantity(commodity.size)}. Its value is {Trace.sim_quantity(commodity.total_value)} and its price is {Trace.sim_quantity(commodity.total_price)}")
    ...
    for stock in Stock.objects.filter(time_stamp_FK=simulation.current_time_stamp):
        unit_price=stock.commodity_FK.unit_price
        unit_value=stock.commodity_FK.unit_value
        Trace.enter(simulation,4,f"Size of {Trace.sim_object(stock.commodity_FK.name)} owned by {Trace.sim_object(stock.stock_owner_name)} is {Trace.sim_quantity(stock.size)}, with value {Trace.sim_quantity(stock.value)} and price {Trace.sim_quantity(stock.price)}")
...
def set_initial_capital(simulation):
    ...
    for industry in Industry.objects.filter(time_stamp_FK=current_time_stamp):
        ...
        for stock in industry.stock_set.filter(time_stamp_FK=current_time_stamp):
            Trace.enter(simulation,3,f"Adding the price {Trace.sim_quantity(stock.price)} of the stock of {Trace.sim_object(stock.commodity_FK.name)}, type {Trace.sim_object(stock.usage_type)}")
            capital+=stock.price
