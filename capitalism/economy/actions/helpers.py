...
def evaluate_commodities(simulation):
    ...
    for commodity in Commodity.objects.filter(time_stamp=current_time_stamp):
        ...
        for stock in stocks:
            ...
            Trace.enter(simulation,3,f"{Trace.sim_object(owner_name)}'s stock of {Trace.sim_object(stock.usage_type)} has added {Trace.sim_quantity(stock.size)} with value {Trace.sim_quantity(stock.value)} and price {Trace.sim_quantity(stock.price)} to commodity {Trace.sim_object(commodity.name)}")
        commodity.save()
    ...
    for commodity in Commodity.objects.filter(time_stamp=current_time_stamp):
        Trace.enter(simulation,2, f"There are now {Trace.sim_quantity(commodity. size)} units of Commodity {Trace.sim_object(commodity.name)} with total value {Trace.sim_quantity(commodity.total_value)} and total price {Trace.sim_quantity(commodity.total_price)}")
        if commodity.size!=0:
            ...
            new_unit_value=1
            new_unit_price=1
        Trace.enter(simulation, 2,f"Unit value {Trace.sim_quantity(commodity.unit_value)} has been reset to {Trace.sim_quantity(new_unit_value)} and unit price {Trace.sim_quantity(commodity.unit_price)} and will be reset to {Trace.sim_quantity(new_unit_price)}")
    ...
    for industry in Industry.objects.filter(time_stamp=current_time_stamp):
        ...
        for stock in industry.stock_set.filter(time_stamp=current_time_stamp):
            ...
            if stock.usage_type==PRODUCTION:
                ...
                Trace.enter(simulation,2,f"Work in progress is now ${Trace.sim_quantity(work_in_progress)}")
        ...
        Trace.enter(simulation,2,f"Capital of {Trace.sim_object(industry.name)} is now ${Trace.sim_quantity(capital)}")        
        Trace.enter(simulation,2,f"Economy-wide initial capital so far is ${Trace.sim_quantity(current_time_stamp.initial_capital)} ")
